...
def html_to_float(c):
    ...
    if not c or c[0] != '#':
        raise ValueError(
            "'%s' must be a valid HTML hex color (e.g., '#f07' or '#d6e7da')" % c
        )
    h = c[1:]
    if len(h) == 3:
...
def rgbcolor(c, space='rgb'):
    ...
    if isinstance(c, str):
        ...
        if len(c) != 3:
            raise ValueError(
                f"color list or tuple '{c}' must have 3 entries, one for each RGB, HSV, HLS, or HSL channel"
            )
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            raise AttributeError(
                "'{}' has no attribute or colormap {}".format(type(self).__name__, name)
            )
...
def get_cmap(cmap):
    ...
    if isinstance(cmap, str):
        ...
        try:
            ...
            raise RuntimeError(
                'Color map %s not known (type "import matplotlib; list(matplotlib.colormaps.keys())" for valid names)'
                % cmap
            )
