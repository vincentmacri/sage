...
class Submodule_free_ambient(Module_free_ambient):
    ...
    def __init__(self, ambient, gens, check=True, already_echelonized=False):
        ...
        if check:
            try:
                ...
                raise TypeError(
                    "each element of basis must be in the ambient free module"
                )
    ...
    def _check_element_membership(self, x):
        ...
        try:
            ...
            raise TypeError("element {} is not in this submodule".format(tuple(x)))
    ...
    def matrix(self):
        ...
        MAT = MatrixSpace(
            self.base_ring(), len(self.gens()), self.degree(), sparse=self.is_sparse()
        )
        A = MAT(self.gens())
        A.set_immutable()
        return A
