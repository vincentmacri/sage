...
class UnramifiedExtensionRingCappedRelative(
    ...
    def __init__(
        ...
        if implementation == 'NTL':
            ntl_poly = ntl_ZZ_pX(
                [a.lift() for a in poly.list()], poly.base_ring().prime() ** prec
            )
            if prec <= 30:
                self.prime_pow = PowComputer_ext_maker(
                    poly.base_ring().prime(),
                    prec,
                    prec,
                    prec,
                    False,
            ...
            self.prime_pow = PowComputer_flint_maker(
                poly.base_ring().prime(),
                cache_limit,
                prec,
                prec,
                False,
                Zpoly,
                prec_type='capped-rel',
            )
            element_class = qAdicCappedRelativeElement
        UnramifiedExtensionGeneric.__init__(
...
class UnramifiedExtensionFieldCappedRelative(
    ...
    def __init__(
        ...
        if implementation == 'NTL':
            ...
            if prec <= 30:
                ...
                self.prime_pow = PowComputer_ext_maker(
                    poly.base_ring().prime(), 30, prec, prec, True, ntl_poly, "big", "u"
                )
            element_class = pAdicZZpXCRElement
        else:
            Zpoly = _make_integral_poly(exact_modulus, poly.base_ring().prime(), prec)
    ...
    def _coerce_map_from_(self, R):
        ...
        """
        if (
            isinstance(R, UnramifiedExtensionRingCappedRelative)
            and R.fraction_field() is self
        ):
            from sage.rings.padics.qadic_flint_CR import pAdicCoercion_CR_frac_field

            return pAdicCoercion_CR_frac_field(R, self)
        if (
            isinstance(R, UnramifiedExtensionRingCappedAbsolute)
            and R.fraction_field() is self
        ):
...
class UnramifiedExtensionRingFloatingPoint(
    ...
    def __init__(
        ...
        self.prime_pow = PowComputer_flint_maker(
            ...
            False,
            Zpoly,
            prec_type='floating-point',
        )
        UnramifiedExtensionGeneric.__init__(
            self, poly, prec, print_mode, names, qAdicFloatingPointElement
        )
...
class UnramifiedExtensionFieldFloatingPoint(
    ...
    def _coerce_map_from_(self, R):
        ...
        """
        if (
            isinstance(R, UnramifiedExtensionRingFloatingPoint)
            and R.fraction_field() is self
        ):
...
class EisensteinExtensionRingCappedRelative(
    ...
    def __init__(
        ...
        ntl_poly = ntl_ZZ_pX(
            [a.lift() for a in poly.list()], poly.base_ring().prime() ** unram_prec
        )
        shift_poly = ntl_ZZ_pX(
            [a.lift() for a in shift_seed.list()],
        ...
        self._exact_modulus = exact_modulus
        self._implementation = implementation
        EisensteinExtensionGeneric.__init__(
...
class EisensteinExtensionRingCappedAbsolute(
    ...
    def __init__(
        self, exact_modulus, poly, prec, print_mode, shift_seed, names, implementation
    ):
        r"""
...
class EisensteinExtensionRingFixedMod(
    ...
    def fraction_field(self):
        ...
        """
        raise TypeError(
            "This implementation of the p-adic ring does not support fields of fractions."
        )
