...
class CNFEncoder:
    ...
    def clauses(self, f):
        ...
        # block of f+1
        return [
            {variable: 1 - value for variable, value in b.items()}
    ...
    def polynomial_clauses(self, f):
        ...
        return [
            product([variable + value for (variable, value) in b.items()])
    ...
    def dimacs_cnf(self, polynomial_system):
        ...
        """
        clauses_list = [
            c for p in polynomial_system for c in self.dimacs_encode_polynomial(p)
        ]
...
class CryptoMiniSatEncoder(CNFEncoder):
    ...
    def dimacs_cnf(self, polynomial_system):
        ...
        res += "\n" + "\n".join(f"c v {self.to_dimacs_index(v)} {v}" for v in uv)
        return res
