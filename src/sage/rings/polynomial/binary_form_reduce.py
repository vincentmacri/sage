        ...
        while err <= zz:
            ...
            if NJinv.base_ring() != CF:
                NJinv = matrix(
                    ...
                    2,
                    2,
                    [CF(zw.numerator() / zw.denominator()) for zw in NJinv.list()],
                )
            w = z
...
def epsinv(F, target, prec=53, target_tol=0.001, z=None, emb=None):
    ...
    def epsF(delta):
        ...
        drts = [
            e
            for e in g.roots(ring=C, multiplicities=False)
            if (e.norm() - 1).abs() < 0.1
        ]
        # find min
    ...
    if max(ex for p, ex in f.roots(ring=C)) >= QQ(d) / 2 or f.degree() < QQ(d) / 2:
...
def smallest_poly(F, prec=53, norm_type='norm', emb=None):
    ...
    if norm_type == 'norm':
        current_size = sum(
            [abs(i) ** 2 for i in G.coefficients()]
        )  # euclidean norm squared
