r"""
Jacobians in the Unique Hess model, binary search variant
...
To create a Jacobian in the Unique Hess model with the binary search variant, specify ``'unique_hess_bs'`` model and provide a base place of degree 1.
...
class JacobianPoint(JacobianPoint_base):
    ...
    def additive_order(self) -> Integer:
        ...
        TESTS::
            ...
            sage: G = C.jacobian(model='unique_hess_bs', base_div=b).group()
            sage: p = C([-1,2,1]).place()
            sage: pt = G.point(p - b)
            sage: pt.order()
            30
            sage: D = pt.divisor()
            sage: G = C.jacobian(model='unique_hess_bs').group()
            sage: P = G(D)
            sage: P.order()
            30
        """
    ...
    def effective_part(self) -> FunctionFieldDivisor:
        ...
        EXAMPLES::
            ...
            sage: C = Curve(x^3 + 5*z^3 - y^2*z, P2)
            sage: b = C([0,1,0]).place()
            sage: G = C.jacobian(model='unique_hess_bs', base_div=b, extra_caching=False).group()
            sage: p = C([-1,2,1]).place()
            sage: pt = G.point(p - b)
...
class JacobianGroup(UniqueRepresentation, JacobianGroup_base):
    ...
    def __init__(self, parent, function_field: FunctionField, base_div: FunctionFieldDivisor) -> None:
        ...
        TESTS::
            ...
            sage: J = F.jacobian(model='unique_hess_bs')
            sage: G = J.group()
    ...
    def _reduce(self, I: FiniteIdeal, J: InfiniteIdeal) -> tuple[FiniteIdeal, InfiniteIdeal, int]:
        ...
        if self._A_is_infinite:
            ...
            C = None
            B_inv = self._inverse_infinite_matrix(J)

        while L < R:
            m = (L + R) // 2
            tilde_I, tilde_J = self._multiply_pair_by_A(I, J, self._A_pows[m])
            C, B_inv, basis = matrices_and_riemann_roch(C, B_inv, tilde_I, tilde_J)
            if basis is None:
                L = m + 1
            else:
                riemann_roch_basis[m] = basis
                R = m

        if R not in riemann_roch_basis:
            tilde_I, tilde_J = self._multiply_pair_by_A(I, J, self._A_pows[R])
            _, _, basis = matrices_and_riemann_roch(C, B_inv, tilde_I, tilde_J)
        else:
            basis = riemann_roch_basis[R]
    ...
    def _use_caching(self, caching: bool):
        ...
        TESTS::
            ...
            sage: J = F.jacobian(model='unique_hess_bs', extra_caching=True)
            sage: G = J.group()
...
class Jacobian(Jacobian_base, UniqueRepresentation):
    ...
    def __init__(self, function_field: FunctionField, base_div: FunctionFieldDivisor | FunctionFieldPlace, cache_infinite_ideals: bool = True, **kwds) -> None:
        ...
        Test projective curve::
            ...
            sage: C = Curve(x^3 + 5*z^3 - y^2*z, P2)
            sage: J = C.jacobian(model='unique_hess_bs')
            sage: TestSuite(J).run()
        """
    ...
    def _repr_(self) -> str:
        ...
        TESTS::
            ...
            sage: P2.<x,y,z> = ProjectiveSpace(GF(7), 2)
            sage: C = Curve(x^3 + 5*z^3 - y^2*z, P2)
            sage: C.jacobian(model='unique_hess_bs')
