...
class FunctionFieldValuationFactory(UniqueFactory):
    ...
    def create_key_and_extra_args(self, domain, prime):
        ...
        if isinstance(prime, tuple):
            if len(prime) == 3:
                ...
                return (
                    self.create_key_and_extra_args_from_valuation_on_isomorphic_field(
                        domain, prime[0], prime[1], prime[2]
                    )
                )
        ...
        if prime.parent() is DiscretePseudoValuationSpace(domain):
            ...
            if not hasattr(prime, "_factory_data"):
                raise NotImplementedError(
                    "Valuations on function fields must be unique and come out of the FunctionFieldValuation factory but %r has been created by other means"
                    % (prime,)
                )
        ...
        raise NotImplementedError(
            "argument must be a place or a pseudo-valuation on a supported subring but %r does not satisfy this for the domain %r"
            % (prime, domain)
        )
    ...
    def create_key_and_extra_args_from_place(self, domain, generator):
        ...
        if generator not in domain.base_field():
            raise NotImplementedError(
                "a place must be defined over a rational function field"
            )

        if domain.base_field() is not domain:
            # if this is an extension field, construct the unique place over
            # the place on the subfield
            return self.create_key_and_extra_args(
                domain, domain.base_field().valuation(generator)
            )
        ...
        if generator in domain._ring:
            ...
            if not generator.is_monic():
                raise ValueError(
                    "place must be defined by a monic polynomial but %r is not monic"
                    % (generator,)
                )
            if not generator.is_irreducible():
                raise ValueError(
                    "place must be defined by an irreducible polynomial but %r factors over %r"
                    % (generator, domain._ring)
                )
            # we construct the corresponding valuation on the polynomial ring
            # with v(generator) = 1
            from sage.rings.valuation.gauss_valuation import GaussValuation

            valuation = GaussValuation(
                domain._ring, TrivialValuation(domain.constant_base_field())
            ).augmentation(generator, 1)
            return self.create_key_and_extra_args(domain, valuation)
        if generator == ~domain.gen():
            # generator is 1/x, the infinite place
            return (
                domain,
                (
                    domain.valuation(domain.gen()),
                    domain.hom(~domain.gen()),
                    domain.hom(~domain.gen()),
                ),
            ), {}
        raise ValueError(
            "a place must be given by an irreducible polynomial or the inverse of the generator; %r does not define a place over %r"
            % (generator, domain)
        )
    ...
    def create_key_and_extra_args_from_valuation(self, domain, valuation):
        ...
        if valuation.domain() is domain._ring:
            if domain.base_field() is not domain:
                ...
                if vK.domain() is not domain.base_field():
                    raise ValueError(
                        "valuation must extend a valuation on the base field but %r extends %r whose domain is not %r"
                        % (valuation, vK, domain.base_field())
                    )
                ...
                approximants = vK.mac_lane_approximants(
                    domain.polynomial(), require_incomparability=True
                )
                approximant = vK.mac_lane_approximant(
                    domain.polynomial(), valuation, approximants
                )
            ...
            if valuation.is_negative_pseudo_valuation():
                raise ValueError(
                    "there must not be an element of valuation -Infinity in the domain of valuation %r"
                    % (valuation,)
                )
        ...
        raise NotImplementedError(
            "extension of valuation from %r to %r not implemented yet"
            % (valuation.domain(), domain)
        )

    def create_key_and_extra_args_from_valuation_on_isomorphic_field(
        self, domain, valuation, to_valuation_domain, from_valuation_domain
    ):
        r"""
        ...
        if valuation.domain() not in FunctionFields():
            raise ValueError(
                "valuation must be defined over an isomorphic function field but %r is not a function field"
                % (valuation.domain(),)
            )
        ...
        if to_valuation_domain not in Hom(domain, valuation.domain()):
            raise ValueError(
                "to_valuation_domain must map from %r to %r but %r maps from %r to %r"
                % (
                    domain,
                    valuation.domain(),
                    to_valuation_domain,
                    to_valuation_domain.domain(),
                    to_valuation_domain.codomain(),
                )
            )
        if from_valuation_domain not in Hom(valuation.domain(), domain):
            raise ValueError(
                "from_valuation_domain must map from %r to %r but %r maps from %r to %r"
                % (
                    valuation.domain(),
                    domain,
                    from_valuation_domain,
                    from_valuation_domain.domain(),
                    from_valuation_domain.codomain(),
                )
            )

        if domain is domain.base():
            if (
                valuation.domain() is not valuation.domain().base()
                or valuation.domain().constant_base_field()
                != domain.constant_base_field()
            ):
                raise NotImplementedError(
                    "maps must be isomorphisms with a rational function field over the same base field, not with %r"
                    % (valuation.domain(),)
                )
            if domain != valuation.domain():
                ...
                raise NotImplementedError(
                    "domain and valuation.domain() must be the same rational function field but %r is not %r"
                    % (domain, valuation.domain())
                )
        else:
            if domain.base() is not valuation.domain().base():
                raise NotImplementedError(
                    "domain and valuation.domain() must have the same base field but %r is not %r"
                    % (domain.base(), valuation.domain().base())
                )
            if to_valuation_domain != domain.hom([to_valuation_domain(domain.gen())]):
                raise NotImplementedError(
                    "to_valuation_domain must be trivial on the base fields but %r is not %r"
                    % (
                        to_valuation_domain,
                        domain.hom([to_valuation_domain(domain.gen())]),
                    )
                )
            if from_valuation_domain != valuation.domain().hom(
                [from_valuation_domain(valuation.domain().gen())]
            ):
                raise NotImplementedError(
                    "from_valuation_domain must be trivial on the base fields but %r is not %r"
                    % (
                        from_valuation_domain,
                        valuation.domain().hom(
                            [from_valuation_domain(valuation.domain().gen())]
                        ),
                    )
                )
            if to_valuation_domain(domain.gen()) == valuation.domain().gen():
                raise NotImplementedError(
                    "to_valuation_domain seems to be trivial but trivial maps would currently break partial orders of valuations"
                )

        if from_valuation_domain(to_valuation_domain(domain.gen())) != domain.gen():
            # only a necessary condition
            raise ValueError(
                "to_valuation_domain and from_valuation_domain are not inverses of each other"
            )
    ...
    def create_object(self, version, key, **extra_args):
        ...
        if isinstance(valuation, tuple) and len(valuation) == 3:
            ...
            if (
                domain is domain.base()
                and valuation.domain() is valuation.domain().base()
            ):
                if valuation == valuation.domain().valuation(valuation.domain().gen()):
                    if to_valuation_domain != domain.hom(
                        [~valuation.domain().gen()]
                    ) or from_valuation_domain != valuation.domain().hom(
                        [~domain.gen()]
                    ):
                        raise ValueError(
                            "the only allowed automorphism for classical valuations is the automorphism x |--> 1/x"
                        )
                ...
                if valuation.is_discrete_valuation():
                    clazz = dynamic_class(
                        "RationalFunctionFieldMappedValuation_discrete",
                        (clazz, DiscreteValuation),
                    )
                else:
                    clazz = dynamic_class(
                        "RationalFunctionFieldMappedValuation_infinite",
                        (clazz, InfiniteDiscretePseudoValuation),
                    )
                return parent.__make_element_class__(clazz)(
                    parent, valuation, to_valuation_domain, from_valuation_domain
                )
            return parent.__make_element_class__(FunctionFieldExtensionMappedValuation)(
                parent, valuation, to_valuation_domain, from_valuation_domain
            )
        ...
        if domain is valuation.domain():
            ...
            raise ValueError(
                "valuation must not be a valuation on domain yet but %r is a valuation on %r"
                % (valuation, domain)
            )
        ...
        if domain.base_field() is domain:
            ...
            if (
                valuation.restriction(domain.constant_base_field()).is_trivial()
                and valuation.is_discrete_valuation()
            ):
            ...
            if valuation.is_discrete_valuation():
                clazz = dynamic_class(
                    "NonClassicalRationalFunctionFieldValuation_discrete",
                    (clazz, DiscreteFunctionFieldValuation_base),
                )
            else:
                clazz = dynamic_class(
                    "NonClassicalRationalFunctionFieldValuation_negative_infinite",
                    (clazz, NegativeInfiniteDiscretePseudoValuation),
                )
            return parent.__make_element_class__(clazz)(parent, valuation)
        # valuation is a limit valuation that singles out an extension
        return parent.__make_element_class__(FunctionFieldFromLimitValuation)(
            parent, valuation, domain.polynomial(), extra_args['approximants']
        )

        raise NotImplementedError(
            "valuation on %r from %r on %r" % (domain, valuation, valuation.domain())
        )


FunctionFieldValuation = FunctionFieldValuationFactory(
    "sage.rings.function_field.valuation.FunctionFieldValuation"
)
            ...
            if K.is_subring(L):
                if L.base() is K:
                    ...
                    if any(self(c) < 0 for c in G.coefficients()):
                        ...
                        if isinstance(y_to_u, RingHomomorphism_im_gens) and isinstance(
                            u_to_y, RingHomomorphism_im_gens
                        ):
                            return [
                                L.valuation(
                                    (
                                        w,
                                        L.hom([M(y_to_u(y_to_u.domain().gen()))]),
                                        M.hom([L(u_to_y(u_to_y.domain().gen()))]),
                                    )
                                )
                                for w in H_extensions
                            ]
                        raise NotImplementedError
                    return [
                        L.valuation(w)
                        for w in self.mac_lane_approximants(
                            L.polynomial(), require_incomparability=True
                        )
                    ]
                if L.base() is not L and K.is_subring(L):
                    # recursively call this method for the tower of fields
                    from functools import reduce
                    from operator import add

                    A = [
                        base_valuation.extensions(L)
                        for base_valuation in self.extensions(L.base())
                    ]
                    return reduce(add, A, [])
                if (
                    L.constant_base_field() is not K.constant_base_field()
                    and K.constant_base_field().is_subring(L)
                ):
                    # subclasses should override this method and handle this case, so we never get here
                    raise NotImplementedError(
                        "Cannot compute the extensions of %r from %r to %r since the base ring changes."
                        % (self, self.domain(), L)
                    )
        raise NotImplementedError(
            "extension of %r from %r to %r not implemented" % (self, K, L)
        )
...
class RationalFunctionFieldValuation_base(FunctionFieldValuation_base):
    ...
    def element_with_valuation(self, s):
        ...
        a, b = self.value_group()._element_with_valuation(
            constant_valuation.value_group(), s
        )
        ret = self.uniformizer() ** a * constant_valuation.element_with_valuation(
            constant_valuation.value_group().gen() * b
        )
...
class ClassicalFunctionFieldValuation_base(DiscreteFunctionFieldValuation_base):
    ...
    def _test_classical_residue_field(self, **options) -> None:
        ...
        tester.assertTrue(
            self.domain().constant_base_field().is_subring(self.residue_field())
        )
...
class InducedRationalFunctionFieldValuation_base(FunctionFieldValuation_base):
    ...
    def __init__(self, parent, base_valuation) -> None:
        ...
        if base_valuation.domain() is not domain._ring:
            raise ValueError(
                "base valuation must be defined on %r but %r is defined on %r"
                % (domain._ring, base_valuation, base_valuation.domain())
            )
    ...
    def lift(self, F):
        ...
        return self.domain()(self._base_valuation.lift(num)) / self.domain()(
            self._base_valuation.lift(den)
        )
    ...
    def _repr_(self) -> str:
        ...
        if isinstance(self._base_valuation, AugmentedValuation_base):
            if self._base_valuation._base_valuation == GaussValuation(
                self.domain()._ring,
                TrivialValuation(self.domain().constant_base_field()),
            ):
        ...
        if self._base_valuation == GaussValuation(self.domain()._ring, vK):
            return repr(vK)
        return (
            "Valuation on rational function field induced by %s" % self._base_valuation
        )
    ...
    def _call_(self, f):
        ...
        EXAMPLES::
            ...
            -2
        """
        return self._base_valuation(f.numerator()) - self._base_valuation(
            f.denominator()
        )
    ...
    def simplify(self, f, error=None, force: bool = False):
        ...
        numerator = self.domain()(
            self._base_valuation.simplify(
                numerator, error=error + v_denominator, force=force
            )
        )
        denominator = self.domain()(
            self._base_valuation.simplify(
                denominator,
                error=max(v_denominator, error - v_numerator + 2 * v_denominator),
                force=force,
            )
        )
    ...
    def _relative_size(self, f):
        ...
        """
        return max(
            self._base_valuation._relative_size(f.numerator()),
            self._base_valuation._relative_size(f.denominator()),
        )
...
class FiniteRationalFunctionFieldValuation(
    ...
    def __init__(self, parent, base_valuation) -> None:
        ...
        TESTS::
            ...
            True
        """
        InducedRationalFunctionFieldValuation_base.__init__(
            self, parent, base_valuation
        )
...
class FunctionFieldFromLimitValuation(
    ...
    def __init__(self, parent, approximant, G, approximants) -> None:
        ...
        TESTS::
            ...
            True
        """
        FiniteExtensionFromLimitValuation.__init__(
            self, parent, approximant, G, approximants
        )
    ...
    def scale(self, scalar):
        ...
        if scalar in QQ and scalar > 0 and scalar != 1:
            return self.domain().valuation(
                self._base_valuation._initial_approximation.scale(scalar)
            )
...
class FunctionFieldMappedValuation_base(
    ...
    def scale(self, scalar):
        ...
        if scalar in QQ and scalar > 0 and scalar != 1:
            return self.domain().valuation(
                (self._base_valuation.scale(scalar), self._to_base, self._from_base)
            )
        return super().scale(scalar)
...
class FunctionFieldMappedValuationRelative_base(FunctionFieldMappedValuation_base):
    ...
    def __init__(
        ...
        TESTS::
            ...
            True
        """
        FunctionFieldMappedValuation_base.__init__(
            self,
            parent,
            base_valuation,
            to_base_valuation_domain,
        ...
        if (
            self.domain().constant_base_field()
            is not base_valuation.domain().constant_base_field()
        ):
            raise ValueError(
                "constant fields must be identical but they differ for %r and %r"
                % (self.domain(), base_valuation.domain())
            )
...
class RationalFunctionFieldMappedValuation(
    ...
    def __init__(
        ...
        TESTS::
            ...
            True
        """
        FunctionFieldMappedValuationRelative_base.__init__(
            self,
            parent,
            base_valuation,
            to_base_valuation_doain,
...
class InfiniteRationalFunctionFieldValuation(
    ...
    def __init__(self, parent) -> None:
        ...
        FunctionFieldMappedValuationRelative_base.__init__(
            self,
            parent,
            FunctionFieldValuation(parent.domain(), x),
            parent.domain().hom([1 / x]),
            parent.domain().hom([1 / x]),
        )
        RationalFunctionFieldValuation_base.__init__(self, parent)
        ClassicalFunctionFieldValuation_base.__init__(self, parent)
