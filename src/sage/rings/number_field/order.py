...
class AbsoluteOrderFactory(OrderFactory):
    ...
    def create_key_and_extra_args(
        self, K, module_rep, is_maximal=None, check=True, is_maximal_at=()
    ):
        r"""
        ...
        if check:
            ...
            if module_rep.rank() != K.degree():
                raise ValueError(
                    "the module defining an absolute order must have full rank"
                )

        return (K, module_rep), {
            "is_maximal": is_maximal,
            "is_maximal_at": {p: True for p in is_maximal_at},
        }
...
class RelativeOrderFactory(OrderFactory):
    ...
    def create_key_and_extra_args(
        self, K, absolute_order, is_maximal=None, check=True, is_maximal_at=()
    ):
        r"""
...
class Order(Parent, sage.rings.abc.Order):
    ...
    def __init__(self, K):
        ...
        Parent.__init__(
            self, base=ZZ, names=K.variable_names(), normalize=False, category=cat
        )
    ...
    def zeta(self, n=2, all=False):
        ...
        if not roots_in_self:
            ...
            raise ArithmeticError(
                "there are no %s roots of unity in self" % n.ordinal_str()
            )
        if all:
    ...
    def residue_field(self, prime, names=None, check=False):
        ...
        raise NotImplementedError(
            "residue fields of non-maximal orders are not yet supported"
        )
    ...
    def conductor(self):
        ...
        if not isinstance(self._K, sage.rings.abc.NumberField_quadratic):
            raise NotImplementedError(
                'not implemented for number fields of degree != 2'
            )
        D = self.discriminant()
...
class Order_absolute(Order):
    ...
    def is_maximal(self, p=None) -> bool:
        ...
        if p is None:
            if self._is_maximal() is None:
                self._assume_maximal(
                    self.absolute_discriminant() == self._K.absolute_discriminant()
                )
            return self._is_maximal()
        p = ZZ(p).abs()

        if self._is_maximal_at(p) is None:
            is_maximal = self._K.maximal_order(
                p, assume_maximal=None
            ).absolute_discriminant().valuation(
                p
            ) == self.absolute_discriminant().valuation(p)
        ...
        if p is None:
            if is_maximal == "non-maximal-non-unique":
                ...
                if self._is_maximal() is False:
                    raise ValueError(
                        "cannot assume this order to be maximal because we already found it to be a non-maximal order"
                    )
                ...
                if self._is_maximal() is True:
                    raise ValueError(
                        "cannot assume this order to be non-maximal because we already found it to be a maximal order"
                    )
                self.__is_maximal = False
        else:
            p = ZZ(p).abs()
            ...
            if is_maximal == "non-maximal-non-unique":
                raise NotImplementedError(
                    "legacy support for explicitly non-maximal orders only possible at all primes"
                )
            elif is_maximal:
                if self._is_maximal_at(p) is False:
                    raise ValueError(
                        f"cannot assume this order to be maximal at {p} because we already found it to be non-maximal at that prime"
                    )
                self.__is_maximal_at[p] = True
            else:
                if self._is_maximal_at(p) is True:
                    raise ValueError(
                        f"cannot assume this order to be non-maximal at {p} because we already found it to be maximal at that prime"
                    )
    ...
    def basis(self):
        ...
        try:
            ...
            B = Sequence(
                [self(from_V(b)) for b in self._module_rep.basis()], immutable=True
            )
            self.__basis = B
        return B
...
class Order_relative(Order):
    ...
    def __add__(left, right):
        ...
        return RelativeOrder(
            left._K, left._absolute_order + right._absolute_order, check=False
        )
    ...
    def _assume_maximal(self, is_maximal=True, p=None):
        ...
        primes::
            ...
            True
        """
        absolute_order = self._absolute_order._assume_maximal(
            is_maximal=is_maximal, p=p
        )
...
def absolute_order_from_ring_generators(
    gens, check_is_integral=True, check_rank=True, is_maximal=None, allow_subfield=False
):
    """
    INPUT:
...
def absolute_order_from_module_generators(
    ...
    return AbsoluteOrder(
        K, W, check=False, is_maximal=is_maximal, is_maximal_at=is_maximal_at
    )
