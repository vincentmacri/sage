...
class DiscretePseudoValuationSpace(UniqueRepresentation, Homset):
    ...
    def _abstract_element_class(self):
        ...
        return dynamic_class(
            class_name, (super()._abstract_element_class, self.__class__.ElementMethods)
        )
    ...
    class ElementMethods:
        ...
        def value_semigroup(self):
            ...
            raise NotImplementedError(
                "cannot determine value semigroup of %r" % (self,)
            )
        ...
        def element_with_valuation(self, s):
            ...
            if s not in self.value_semigroup():
                raise ValueError(
                    "s must be in the value semigroup of this valuation but %r is not in %r"
                    % (s, self.value_semigroup())
                )
            if s == 0:
            ...
            if exp not in ZZ:
                raise NotImplementedError(
                    "s must be a multiple of %r but %r is not"
                    % (self.value_group().gen(), s)
                )
        ...
        def residue_field(self):
            ...
            if isinstance(ret, PolynomialRing_generic):
                ...
                return FunctionField(
                    ret.base_ring().fraction_field(), names=(ret.variable_name(),)
                )
        ...
        def extension(self, ring):
            ...
            if len(extensions) > 1:
                raise ValueError(
                    "there is no unique extension of %r from %r to %r"
                    % (self, self.domain(), ring)
                )
            return extensions[0]
        ...
        def extensions(self, ring):
            ...
            if ring is self.domain():
                return [self]
            raise NotImplementedError(
                "extending %r from %r to %r not implemented"
                % (self, self.domain(), ring)
            )
        ...
        def restriction(self, ring):
            ...
            if ring is self.domain():
                return self
            raise NotImplementedError(
                "restricting %r from %r to %r not implemented"
                % (self, self.domain(), ring)
            )
        ...
        def change_domain(self, ring):
            ...
            raise NotImplementedError(
                "changing %r from %r to %r not implemented"
                % (self, self.domain(), ring)
            )
        ...
        def separating_element(self, others):
            ...
            for other in others + [self]:
                if other.parent() is not self.parent():
                    raise ValueError(
                        "all valuations must be valuations on %r but %r is a valuation on %r"
                        % (self.domain(), other, other.domain())
                    )
                if not other.is_discrete_valuation():
                    raise ValueError(
                        "all valuations must be discrete valuations but %r is not"
                        % (other,)
                    )
                if other.is_trivial():
                    raise ValueError(
                        "all valuations must be non-trivial but %r is not" % (other,)
                    )
        ...
        def _weakly_separating_element(self, other):
            ...
            if self(ret) > other(ret):
                return ret
            raise NotImplementedError(
                "weakly separating element for %r and %r" % (self, other)
            )
        ...
        def shift(self, x, s):
            ...
            for i in range(-s):
                if self(x) < 0:
                    raise NotImplementedError(
                        "cannot compute general shifts over non-fields which contain elements of negative valuation"
                    )
            ...
            for x, s in tester.some_elements(product(X, S)):
                ...
                if self.domain().is_exact():
                    ...
                    tester.assertGreaterEqual(
                        self(x - y), min(s, y.precision_absolute())
                    )
        ...
        def _test_scale(self, **options):
            ...
            for s in tester.some_elements(QQ.some_elements()):
                ...
                if not self.is_trivial():
                    ...
                    tester.assertEqual(
                        scaled(self.uniformizer()), s * self(self.uniformizer())
                    )
                unscaled = scaled / s
