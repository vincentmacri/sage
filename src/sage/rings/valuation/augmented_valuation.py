...
from .inductive_valuation import (
    ...
    def create_key(self, base_valuation, phi, mu, check=True):
        ...
        if check:
            ...
            if mu <= base_valuation(phi):
                raise ValueError(
                    "the value of the key polynomial must strictly increase but `%s` does not exceed `%s`."
                    % (mu, base_valuation(phi))
                )
        ...
        if isinstance(base_valuation, AugmentedValuation_base):
            if phi.degree() == base_valuation.phi().degree():
                ...
                return self.create_key(
                    base_valuation._base_valuation, phi, mu, check=check
                )
    ...
    def create_object(self, version, key):
        ...
        if mu is not infinity:
            if base_valuation.is_trivial():
                return parent.__make_element_class__(FinalFiniteAugmentedValuation)(
                    parent, base_valuation, phi, mu
                )
            return parent.__make_element_class__(NonFinalFiniteAugmentedValuation)(
                parent, base_valuation, phi, mu
            )
        return parent.__make_element_class__(InfiniteAugmentedValuation)(
            parent, base_valuation, phi, mu
        )


AugmentedValuation = AugmentedValuationFactory(
    "sage.rings.valuation.augmented_valuation.AugmentedValuation"
)
...
class AugmentedValuation_base(InductiveValuation):
    ...
    def equivalence_unit(self, s, reciprocal=False):
        ...
        if reciprocal:
            ...
            residue = self.reduce(
                ret * self._base_valuation.element_with_valuation(-s), check=False
            )
    ...
    def element_with_valuation(self, s):
        ...
        if s not in self.value_group():
            raise ValueError(
                "s must be in the value group of the valuation but %r is not in %r."
                % (s, self.value_group())
            )
        error = s
    ...
    def _repr_(self):
        ...
        vals = [
            "v(%s) = %s" % (v._phi, v._mu)
            if isinstance(v, AugmentedValuation_base)
            else str(v)
            for v in vals
        ]
        return "[ %s ]" % ", ".join(vals)
    ...
    def E(self):
        ...
        if self.augmentation_chain()[-1]._base_valuation.is_trivial():
            raise NotImplementedError(
                "ramification index is not defined over a trivial Gauss valuation"
            )
        return (
            self.value_group().index(self._base_valuation.value_group())
            * self._base_valuation.E()
        )
                ...
                if v.is_key(phi):
                    ...
                    for f, e in F:
                        ...
                        mu = (
                            self._mu
                            - v(F.unit())
                            - sum([ee * v(ff) for ff, ee in F if ff != f])
                        ) / e
        ...
        if ring.is_subring(self.domain()):
            ...
            if isinstance(ring, PolynomialRing_generic):  # univariate
                return base.augmentation(
                    self.phi().change_ring(ring.base_ring()), self._mu
                )
    ...
    def scale(self, scalar):
        ...
        if scalar in QQ and scalar > 0 and scalar != 1:
            return self._base_valuation.scale(scalar).augmentation(
                self.phi(), scalar * self._mu
            )
    ...
    def change_domain(self, ring):
        ...
        if (
            ...
            and ring.variable_name() == self.domain().variable_name()
        ):
            return self._base_valuation.change_domain(ring).augmentation(
                self.phi().change_ring(ring.base_ring()), self._mu, check=False
            )
...
class FinalAugmentedValuation(AugmentedValuation_base, FinalInductiveValuation):
    ...
    def lift(self, F):
        ...
        if self.psi().degree() > 1:
            ...
            from sage.rings.function_field.element_polymod import (
            ...
            F = G.change_variable_name(
                self._base_valuation.residue_ring().variable_name()
            )
...
class NonFinalAugmentedValuation(AugmentedValuation_base, NonFinalInductiveValuation):
    ...
    def reduce(
        ...
        for i, c in enumerate(coefficients):
            if i % tau != 0:
                ...
                if i // tau >= len(valuations):
                    ...
                    valuations.append(
                        self._base_valuation.lower_bound(c) + i * self._mu
                    )
                v = valuations[i // tau]
        ...
        C = [
            self._base_valuation.reduce(c, check=False)(self._residue_field_generator())
    ...
    def lift(self, F, report_coefficients=False):
        ...
        coeffs = [
            R0(
                c
                if self.psi().degree() == 1
                else list(c._vector_() if hasattr(c, '_vector_') else c.list())
            )
        ...
        coeffs = [
            (c if i == 0 else c * self._Q_reciprocal(i)).map_coefficients(
                _lift_to_maximal_precision
            )
    ...
    def lift_to_key(self, F, check=True):
        ...
        coefficients = [c * self._Q(F.degree()) for i, c in enumerate(coefficients)] + [
            self.domain().one()
        ]
    ...
    def _Q_reciprocal(self, e=1):
        ...
        assert self._base_valuation.reduce(self._Q(e) * ret)(
            self._residue_field_generator()
        ).is_one()
...
class FiniteAugmentedValuation(AugmentedValuation_base, FiniteInductiveValuation):
    ...
    def simplify(
        ...
        if effective_degree is not None:
            if (QQ(f.degree()) / self.phi().degree()).ceil() > effective_degree:
                f = self.domain().change_ring(self.domain())(
                    list(islice(self.coefficients(f), 0, int(effective_degree) + 1, 1))
                )(self.phi())

        if f.degree() < self.phi().degree():
            return self._base_valuation.simplify(
                f,
                error=error,
                force=force,
                size_heuristic_bound=size_heuristic_bound,
                phiadic=phiadic,
            )
        ...
        if phiadic or self.phi() == self.phi().parent().gen():
            ...
            return self.domain().change_ring(self.domain())(
                [
                    0
                    if valuations[i] > error
                    else self._base_valuation.simplify(
                        c, error=error - i * self._mu, force=force, phiadic=True
                    )
                    for (i, c) in enumerate(coefficients)
                ]
            )(self.phi())
        ...
        for i in range(f.degree(), -1, -1):
            ...
            coefficients = list(
                islice(f.list(), int(j * self.phi().degree()), int(i) + 1)
            )
            g = self.domain()(coefficients)
            ng = self._base_valuation.simplify(
                g, error=error - j * self._mu, force=force, phiadic=False
            )
            if g != ng:
                f -= (g - ng) * self.phi() ** j
        return f
    ...
    def upper_bound(self, f):
        ...
        return (
            self.restriction(f.base_ring())(f.leading_coefficient())
            + len_coefficients_bound * self._mu
        )
    ...
    def simplify(self, f, error=None, force=False, effective_degree=None):
        ...
        return self.domain()(
            self._base_valuation.simplify(
                next(self.coefficients(f)), error=error, force=force
            )
        )
