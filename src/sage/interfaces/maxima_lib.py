...
ecl_eval(
    r"(defun tex-derivative (x l r) (tex (if $derivabbrev (tex-dabbrev x) (tex-d x '\\partial)) l r lop rop ))"
)
ecl_eval(
    '(defun principal nil (cond ($noprincipal (diverg)) ((not pcprntd) (merror "Divergent Integral"))))'
)
...
if MAXIMA_PREFIX:
    ...
    ecl_eval(
        f'#$file_search_maxima: append(file_search_maxima, ["{MAXIMA_PREFIX}/###.{{mac,mc,wxm}}", "{MAXIMA_PREFIX}/{import_packages}/###.{{mac,mc,wxm}}"])$'
    )
    ecl_eval(
        f'#$file_search_lisp: append(file_search_lisp, ["{MAXIMA_PREFIX}/###.{{fas,lisp,lsp}}", "{MAXIMA_PREFIX}/../src/###.{{fas,lisp,lsp}}", "{MAXIMA_PREFIX}/{import_packages}/###.{{fas,lisp,lsp}}"])$'
    )
...
class MaximaLib(MaximaAbstract):
    ...
    def __init__(self):
        ...
        if maxima_lib_instances > 0:
            raise RuntimeError(
                "Maxima interface in library mode can only be instantiated once"
            )
    ...
    def sr_integral(self, *args):
        ...
        try:
            return max_to_sr(
                maxima_eval(([max_integrate], [sr_to_max(SR(a)) for a in args]))
            )
        except RuntimeError as error:
            s = str(error)
            ...
            return max_to_sr(
                maxima_eval(
                    [
                        ...
                        [
                            [max_simplify_sum],
                            ([max_sum], [sr_to_max(SR(a)) for a in args]),
                        ],
                    ]
                )
            )
            ...
            return max_to_sr(
                maxima_eval(
                    [
                        ...
                        [
                            [max_simplify_prod],
                            ([max_prod], [sr_to_max(SR(a)) for a in args]),
                        ],
                    ]
                )
            )
    ...
    def _missing_assumption(self, errstr):
        ...
        outstr = (
            "Computation failed since Maxima requested additional constraints; using the 'assume' command before evaluation *may* help (example of legal syntax is 'assume("
            + errstr[jj + 1 : k]
...
def mqapply_to_sage(expr):
    ...
    if caaadr(expr) == max_li:
        return sage.functions.log.polylog(
            max_to_sr(cadadr(expr)), max_to_sr(caddr(expr))
        )
    if caaadr(expr) == max_psi:
        return sage.functions.gamma.psi(max_to_sr(cadadr(expr)), max_to_sr(caddr(expr)))
    if caaadr(expr) == max_hyper:
        return sage.functions.hypergeometric.hypergeometric(
            mlist_to_sage(car(cdr(cdr(expr)))),
...
def max_harmonic_to_sage(expr):
    ...
    EXAMPLES::
        ...
        harmonic_number(x, 2)
    """
    return sage.functions.log.harmonic_number(
        max_to_sr(caddr(expr)), max_to_sr(cadr(expr))
    )
...
special_sage_to_max = {
    ...
    sage.functions.log.lambert_w: lambda N, X: (
        [[max_lambert_w], X]
        if N == EclObject(0)
        else [[mqapply], [[max_lambert_w, max_array], N], X]
    ),
    sage.functions.log.harmonic_number: lambda N, X: [[max_harmo], X, N],
    sage.functions.hypergeometric.hypergeometric: lambda A, B, X: [
        [mqapply],
        [[max_hyper, max_array], lisp_length(A.cdr()), lisp_length(B.cdr())],
        A,
        B,
        X,
    ],
...
def pyobject_to_max(obj):
    ...
    if isinstance(obj, sage.rings.rational.Rational):
        return (
            EclObject(obj)
            if (obj.denom().is_one())
            else EclObject([[rat], obj.numer(), obj.denom()])
        )
    if isinstance(obj, NumberFieldElement_base):
        ...
        if isinstance(
            obj, NumberFieldElement_quadratic
        ) and obj.parent().defining_polynomial().list() == [1, 0, 1]:
            re, im = obj.list()
            return EclObject(
                [[mplus], pyobject_to_max(re), [[mtimes], pyobject_to_max(im), max_i]]
            )
...
def sr_to_max(expr):
    ...
    if op:
        ...
        if isinstance(op, FDerivativeOperator):
            args = expr.operands()
            if not all(
                isinstance(v, Expression) and v.is_symbol() for v in args
                ...
                for i in set(params):
                    deriv_max.extend(
                        [sr_to_max(temp_args[i]), EclObject(params.count(i))]
                    )
            ...
            deriv_max = []
            [
                deriv_max.extend([sr_to_max(args[i]), EclObject(params.count(i))])
                for i in set(params)
            ]
            l = [[mdiff], f]
            l.extend(deriv_max)
        ...
        if op in special_sage_to_max:
            return EclObject(
                special_sage_to_max[op](*[sr_to_max(o) for o in expr.operands()])
            )
        if op is tuple:
        ...
        if op not in sage_op_dict:
            ...
            if op_max in max_op_dict:
                raise RuntimeError(
                    "Encountered operator mismatch in sr-to-maxima translation"
                )
            sage_op_dict[op] = op_max
            max_op_dict[op_max] = op
        return EclObject(([sage_op_dict[op]], [sr_to_max(o) for o in expr.operands()]))
    ...
    if expr.consp():
        ...
        if op_max not in max_op_dict:
            ...
            if op in sage_op_dict:
                raise RuntimeError(
                    "Encountered operator mismatch in maxima-to-sr translation"
                )
            max_op_dict[op_max] = op
            sage_op_dict[op] = op_max
        else:
...
max_is = EclObject("$IS")
test_max_equal = lambda A, B: maxima_eval(
    [[max_is], [[max_equal], sr_to_max(A), sr_to_max(B)]]
).python()
test_max_notequal = lambda A, B: maxima_eval(
    [[max_is], [[max_notequal], sr_to_max(A), sr_to_max(B)]]
).python()
