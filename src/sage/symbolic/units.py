...
unitdict = {
    ...
    'amount_of_substance': {'elementary_entity': '1/6.02214129000000e23', 'mole': 1},
    'angles': {
    ...
    'conductance': {'abmho': 1000000000, 'mho': 1, 'siemens': 1},
    'current': {
        'abampere': 10,
        'amp': 1,
        'ampere': 1,
    ...
    'fiber_linear_mass_density': {'denier': one / 9000000, 'tex': one / 1000000},
    'force': {
        'dyne': one / 100000,
    ...
    'frequency': {'1/second': 1, 'hertz': 1},
    'illuminance': {'foot_candle': QQ(1562500) / 145161, 'lux': 1, 'phot': 10000},
    'inductance': {
        'abhenry': one / 1000000000,
        'henry': 1,
        'stathenry': QQ(22468879468420441) / 25000,
    },
    'information': {'bit': 1, 'byte': 8, 'nibble': 4},
    'information_rate': {'baud': 1},
    'inverse_length': {'diopter': 1, 'kayser': 100},
    'length': {
    ...
    'luminance': {'apostilb': '1/pi', 'lambert': '10000/pi', 'nit': 1, 'stilb': 10000},
    'luminous_energy': {'lumerg': 1, 'talbot': 1},
    'luminous_flux': {'lumen': 1},
    'luminous_intensity': {'candela': 1, 'candle': 1, 'hefnerkerze': QQ(1019) / 1128},
    'magnetic_field': {'gauss': one / 10000, 'tesla': 1},
    'magnetic_flux': {'maxwell': one / 100000000, 'weber': 1},
    'magnetic_intensity': {'oersted': '250/pi'},
    'magnetic_moment': {
        'bohr_magneton': '9.27400915000000e-24',
        'nuclear_magneton': '5.05078324000000e-27',
    },
    'magnetomotive_force': {'ampere_turn': 1, 'gilbert': '5/2/pi'},
    'mass': {
    ...
    'radiation': {'becquerel': 1, 'curie': 37000000000, 'rutherford': 1000000},
    'radiation_absorbed': {'gray': 1, 'rad': one / 100},
    'radiation_ionizing': {
    ...
    'velocity': {'knot': '463/900'},
    'viscosity_absolute': {'poise': one / 10, 'reyn': QQ(8896443230521) / 1290320000},
    'viscosity_kinematic': {'stokes': one / 10000},
    'viscosity_other': {'rhes': 10},
    'volume': {
...
def evalunitdict():
    ...
    for key, value in unitdict.items():
        unitdict[key] = {
            a: (sage_eval(repr(b)) if isinstance(b, str) else b)
...
unit_docs = {
    ...
    'information_rate_docs': {'baud': 'Defined to be 1 bit/second.'},
    'inverse_length_docs': {
    ...
    'viscosity_other_docs': {'rhes': 'Defined to be 1/poise or 10/(pascal*second).'},
    'volume_docs': {
...
class Units(ExtraTabCompletion):
    ...
    def __repr__(self):
        ...
        return "Collection of units{0}: {1}".format(
            name, ' '.join(sorted([str(x) for x in self.__data]))
        )
...
def convert_temperature(expr, target):
    ...
    if len(expr.variables()) != 1:
        ...
        if (
            target_temp == units.temperature.celsius
            or target_temp == units.temperature.centigrade
        ):
