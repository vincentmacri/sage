...
class CryptoMiniSat(SatSolver):
    ...
    def __init__(self, verbosity=0, confl_limit=None, threads=None) -> None:
        ...
        EXAMPLES::
            ...
            sage: # optional - pycryptosat
            sage: from sage.sat.solvers.cryptominisat import CryptoMiniSat
            sage: solver = CryptoMiniSat(threads=1)
        """
        if threads is None:
    ...
    def var(self, decision=None) -> int:
        r"""
        Return a *new* variable.

        INPUT:

        - ``decision`` -- for compatibility with other solvers; ignored
    ...
    def add_xor_clause(self, lits, rhs=True) -> None:
        ...
        - ``rhs`` -- boolean (default: ``True``); whether this XOR
          clause should be evaluated to ``True`` or ``False``
        ...
        EXAMPLES::
            ...
            sage: from sage.sat.solvers.cryptominisat import CryptoMiniSat
            sage: solver = CryptoMiniSat()
            sage: solver.add_xor_clause((1, 2 , 3), False)
        """
        if 0 in lits:
            raise ValueError(f"0 should not appear in the clause: {lits}")
        # cryptominisat does not handle Sage integers
    ...
    def __repr__(self) -> str:
        ...
        """
        nc = len(self.clauses())
        return f"CryptoMiniSat solver: {self.nvars()} variables, {nc} clauses."
    ...
    def clauses(self, filename=None):
        ...
        - ``filename`` -- if not ``None`` clauses are written to
          ``filename`` in DIMACS format (default: ``None``)
        ...
        Note that in cryptominisat, the DIMACS standard format is
        augmented with the following extension: having an ``x`` in
        front of a line makes that line an XOR clause::

            sage: # optional - pycryptosat
            sage: solver.add_xor_clause((1,2,3), rhs=True)
            sage: solver.clauses(fn)
            sage: print(open(fn).read())
            p cnf 4 3
            1 2 4 0
        ...
        variables::
            ...
            sage: # optional - pycryptosat
            sage: solver.add_xor_clause((1,2,5),rhs=False)
            sage: solver.clauses(fn)
            sage: print(open(fn).read())
            p cnf 5 4
            1 2 4 0
