...
class LieSubalgebra_finite_dimensional_with_basis(Parent, UniqueRepresentation):
    ...
    def __classcall_private__(
        cls, ambient, gens, ideal_of=None, order=None, category=None
    ):
        """
    ...
    def _repr_(self):
        ...
        return "Subalgebra generated by {} of {}".format(
            self._repr_short(), self.ambient()
        )
    ...
    def _to_m(self, X):
        ...
        B = M.basis()
        return M.sum(
            R(mc[self._reversed_indices[i]]) * B[i]
            for i in range(len(B))
            if self._reversed_indices[i] in mc
        )
    ...
    def _from_m(self, v):
        ...
        B = M.basis()
        v_sorted = M.sum(
            R(v_self[self._reorganized_indices[i]]) * B[i] for i in range(len(B))
        )
    ...
    def retract(self, X):
        ...
        Retraction to a subalgebra of a free nilpotent Lie algebra::
            ...
            2*X_1 + 3*X_2 + 5*X_12
            sage: el.parent()
            Subalgebra generated by (X_1, X_2) of Free Nilpotent Lie algebra
             of rank 3 and step 2 over Rational Field
        ...
        subalgebra::
            ...
            ValueError: the element X_3 is not in Subalgebra generated
             by (X_1, X_2) of Free Nilpotent Lie algebra of rank 3 and
             step 2 over Rational Field
        """
        X = self._ambient(X)
        if X not in self:
    ...
    def basis(self):
        ...
        while sm.dimension() > d:
            ...
            brackets = [
                self._to_m(ambient.bracket(self._from_m(v), self._from_m(w)))
                for v in B
                for w in SB
            ]
        ...
        basis = [
            self.element_class(self, self._from_m(v)) for v in sm.echelonized_basis()
        ]
    ...
    def leading_monomials(self):
        ...
        """
        return Family(
            self.lift(X).leading_monomial(key=self._order) for X in self.basis()
        )
