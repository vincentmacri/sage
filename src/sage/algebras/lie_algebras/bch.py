...
def bch_iterator(X=None, Y=None):
    ...
    .. SEEALSO::
        ...
        To get sums of terms in the BCH formula, use the method
        :meth:`~sage.categories.lie_algebras.LieAlgebras.ParentMethods.bch()`
        for any Lie algebra.
    ...
    .. WARNING::
        ...
        The time needed to compute each successive term increases
        exponentially. For example on one machine iterating through
        `Z_{11}, \ldots, Z_{18}` for a free Lie algebra, computing
        each successive term took 4-5 times longer, going from 0.1s
        for `Z_{11}` to 21 minutes for `Z_{18}`.
    ...
    We compute the full BCH formula for a free nilpotent Lie algebra
    (as a way to approximate the general computation)::
    ...
    step = None
    if L in LieAlgebras.Nilpotent:
        step = L.step()

    while True:
        m += 1
        if step is not None and m > step:
            return
