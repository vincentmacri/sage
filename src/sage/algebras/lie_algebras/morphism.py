...
class LieAlgebraHomomorphism_im_gens(Morphism):
    ...
    def __init__(self, parent, im_gens, base_map=None, check=True):
        ...
        if check:
            ...
            if base_map is not None and not (
                ...
                and parent.codomain()
                .base_ring()
                .has_coerce_map_from(base_map.codomain())
            ):
    ...
    def _richcmp_(self, other, op):
        ...
        """
        return richcmp(
            (self._im_gens, self._base_map), (other._im_gens, other._base_map), op
        )
    ...
    def _repr_defn(self):
        ...
        D = self.domain()
        s = '\n'.join(
            '%s |--> %s' % (x, gen) for gen, x in zip(self._im_gens, D.gens())
        )
...
class LieAlgebraHomset(Homset):
    ...
    def __call__(self, im_gens, check=True):
        ...
        if (
            self.domain() in LieAlgebras.FiniteDimensional.WithBasis
            and self.codomain() in LieAlgebras.FiniteDimensional.WithBasis
        ):
            try:
                return LieAlgebraMorphism_from_generators(
                    self.domain(), im_gens, codomain=self.codomain(), check=check
                )
...
class LieAlgebraMorphism_from_generators(LieAlgebraHomomorphism_im_gens):
    ...
    def __init__(
        ...
        if not im_gens:
            LieAlgebraHomomorphism_im_gens.__init__(
                self, parent, [], base_map=base_map, check=check
            )
            return
        ...
        def solve_linear_system(A, b, check):
            ...
            return [
                sum((R(Aij) * bk for Aij, bk in zip(Ai, b)), cm.zero())
        ...
        while True:
            ...
            try:
                ...
                raise ValueError(
                    "this does not define a Lie algebra morphism; "
                    "contradictory values for brackets of length %d" % bracketlength
                )
        ...
        if not sm.has_coerce_map_from(m):
            raise ValueError(
                "%s is not a generating set of %s" % (list(on_generators), domain)
            )
