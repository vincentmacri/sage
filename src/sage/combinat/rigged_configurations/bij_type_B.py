    ...
    def run(self, verbose=False):
        ...
        for cur_crystal in reversed(self.tp_krt):
            ...
            if r == self.n:
                ...
                from sage.combinat.rigged_configurations.bij_type_A2_odd import (
                    KRTToRCBijectionTypeA2Odd,
                )
                ...
                bij.ret_rig_con = KRT.rigged_configurations()(
                    *self.ret_rig_con, use_vacancy_numbers=True
                )
                ...
                for col_number, cur_column in enumerate(
                    ...
                    for letter in reversed(cur_column):
                        ...
                        val = (
                            letter.value
                        )  # Convert from a CrystalOfLetter to an Integer
                ...
                self.ret_rig_con = self.tp_krt.parent().rigged_configurations()(
                    *bij.ret_rig_con, use_vacancy_numbers=True
                )
        ...
        for i in range(num_rows + 1):
            if i == num_rows:
                ...
                if p._list[i] == max_width and not singular_max_width:
                    p._list[i] += (
                        1  # We always at least add a box to the first singular value
                    )
                    p.rigging[i] = None
                    if case_QS:
                        width_n = p._list[i]
                ...
                p.vacancy_numbers[i] - 1 == p.rigging[i]
                and not case_QS
                and not singular_max_width
                and p._list[i] <= max_width
            ):
                case_QS = True
                max_width = p._list[i]
                p._list[i] += 1
    ...
    def run(self, verbose=False, build_graph=False):
        ...
        for dim in self.rigged_con.parent().dims:
            ...
            if dim[0] == self.n:
                ...
                bij = RCToKRTBijectionTypeA2Odd(
                    RC(*self.cur_partitions, use_vacancy_numbers=True)
                )
                for i in range(len(self.cur_dims)):
                    if bij.cur_dims[i][0] != self.n:
                        bij.cur_dims[i][1] *= 2
                ...
                for dummy_var in range(dim[1]):
                    ...
                    while bij.cur_dims[0][0]:  # > 0:
                        if verbose:
                            print("====================")
                            print(
                                repr(RC(*bij.cur_partitions, use_vacancy_numbers=True))
                            )
                            print("--------------------")
                            print(ret_crystal_path)
                            print("--------------------\n")
                    ...
                    print(
                        repr(
                            self.rigged_con.parent()(
                                *bij.cur_partitions, use_vacancy_numbers=True
                            )
                        )
                    )
                    print("--------------------")
    ...
    def next_state(self, height):
        ...
        if b is None:
            ...
            for i in reversed(range(len(partition))):
                if (
                    ...
                    if (
                        not case_Q
                        and partition.vacancy_numbers[i] - 1 == partition.rigging[i]
                    ):
                        case_Q = True
                        ...
                        if (
                            not case_Q
                        ):  # We found a singular string above the quasi-singular one
                            break
                        ell[n - 1] = i
        ...
        if row_num_bar_next is not None:  # If we enter here, it means case (Q, S) holds
            ...
            if make_quasisingular:
                ...
                while (
                    j < length
                    and self.cur_partitions[n - 1][j] == block_len
