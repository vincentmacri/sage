...
def init():
    ...
    for other_basis, other_name in translate.items():
        for basis, name in translate.items():
            try:
                conversion_functions[(other_basis, basis)] = getattr(
                    symmetrica, f't_{other_name}_{name}'
                )
...
class SymmetricFunctionAlgebra_classical(sfa.SymmetricFunctionAlgebra_generic):
    ...
    def _element_constructor_(self, x):
        ...
        # Dual bases #
        ##############
        if isinstance(x, sfa.SymmetricFunctionAlgebra_generic.Element) and hasattr(
            x, 'dual'
        ):
        ...
        if isinstance(x, self.Element):
            ...
            return eclass(
                self,
                {la: rc for la, c in x._monomial_coefficients.items() if (rc := R(c))},
            )
        ...
        if isinstance(x, SymmetricFunctionAlgebra_classical.Element):
            ...
            try:
                ...
                raise TypeError(
                    "do not know how to convert from %s to %s"
                    % (P.basis_name(), self.basis_name())
                )

            if R == QQ and P.base_ring() == QQ:
                if m:
                    return self._from_dict(t(m)._monomial_coefficients, coerce=True)
                return self.zero()
        ...
        if isinstance(x, hall_littlewood.HallLittlewood_generic.Element):
            ...
            if isinstance(
                x,
                (
                    hall_littlewood.HallLittlewood_qp.Element,
        ...
        # Convert to m and then to self.
            ...
            for m, c in x._monomial_coefficients.items():
                ...
                for part in P._self_to_m_cache[n][m]:
                    z_elt[part] = z_elt.get(part, zero) + R(
        ...
        #########################
            ...
            if isinstance(
                x,
                (
                    macdonald.MacdonaldPolynomials_j.Element,
            ...
            if isinstance(
                x,
                (
                    macdonald.MacdonaldPolynomials_q.Element,
            ...
            if isinstance(
                x,
                (
                    macdonald.MacdonaldPolynomials_h.Element,
        ...
        ####################
            ...
            if isinstance(x, jack.JackPolynomials_p.Element):
                ...
                return self(mx)
            if isinstance(
                x, (jack.JackPolynomials_j.Element, jack.JackPolynomials_q.Element)
            ):
                return self(x.parent()._P(x))
            raise TypeError
        ...
        #################################
            ...
            try:
                ...
                raise TypeError(
                    "do not know how to make x (= {}) an element of self".format(x)
                )
            else:
                if not c:
                    return self.zero()
