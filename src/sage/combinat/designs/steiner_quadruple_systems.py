            ...
            for bb in range(n - 4):
                ...
                for d in range(2 * k + 1):
                    Y.append(
                        [
                            r(i + 2, bbb),
                            r(i, b + 2 * k + 1 + i * (4 * k + 2) - d),
                            r(i, b + 2 * k + 2 + i * (4 * k + 2) + d),
                            r(i + 1, bb),
                        ]
                    )
            ...
            for b in range(12):
                for bb in range(12):
                    ...
                    for h in range(2):
                        # Line 2
                        Y.append(
                            [r(n - 2, 11) + 1 + h, r(u, b), r(v, bb), r(w, bbb + 3 * h)]
                        )
...
def P(alpha, m):
    ...
    if m % 2 == 0:
        ...
        pairs += [
            (c, (2 * m + 2 * y - c - 2) % (2 * m)) for c in range(2 * y + 1, m + y - 1)
        ]
        pairs += [
            (2 * m + int(-1.5 - 0.5 * (-1) ** y), y),
            (2 * m + int(-1.5 + 0.5 * (-1) ** y), m + y - 1),
        ]
        return pairs
    if alpha < m - 1:
        if alpha % 2 == 0:
        ...
        for n in range(1, (m - 2) // 2 + 1):
            pairs.append(
                [
                    p
                    for p in P(2 * n, m)
                    if not isequal(p, (2 * n, (4 * n + 1) % (2 * m)))
                ]
            )
            ...
            pairs.append(
                [
                    p
                    for p in P(2 * n - 1, m)
                    if not isequal(p, (2 * m - 2 - 2 * n, 2 * m - 1 - 4 * n))
                ]
            )
        ...
        for n in range((m - 3) // 2 + 1):
            ...
            pairs.append(
                [
                    p
                    for p in P(2 * n + 1, m)
                    if not isequal(p, (2 * m - 2 - 2 * n, 2 * m - 3 - 4 * n))
                ]
            )
