...
def construction_3_3(k, n, m, i, explain_construction=False):
    ...
    if explain_construction:
        return (
            "Construction 3.3 with n={},m={},i={} from:\n"
            "  Julian R. Abel, Nicholas Cavenagh\n"
            + "  Concerning eight mutually orthogonal latin squares,\n"
            + "  Vol. 15, n.3, pp. 255-261,\n"
            + "  Journal of Combinatorial Designs, 2007"
...
def construction_3_4(k, n, m, r, s, explain_construction=False):
    ...
    if explain_construction:
        return (
            "Construction 3.4 with n={},m={},r={},s={} from:\n"
            + "  Julian R. Abel, Nicholas Cavenagh\n"
            + "  Concerning eight mutually orthogonal latin squares,\n"
            + "  Vol. 15, n.3, pp. 255-261,\n"
            + "  Journal of Combinatorial Designs, 2007"
...
def construction_3_5(k, n, m, r, s, t, explain_construction=False):
    ...
    if explain_construction:
        return (
            "Construction 3.5 with n={},m={},r={},s={},t={} from:\n"
            "  Julian R. Abel, Nicholas Cavenagh\n"
            + "  Concerning eight mutually orthogonal latin squares,\n"
            + "  Vol. 15, n.3, pp. 255-261,\n"
            + "  Journal of Combinatorial Designs, 2007"
    ...
    group_k_3 = [
        ...
        if B[-3] not in group_k_1 and B[-2] not in group_k_2
    ]
...
def construction_3_6(k, n, m, i, explain_construction=False):
    ...
    if explain_construction:
        return (
            "Construction 3.6 with n={},m={},i={} from:\n"
            "  Julian R. Abel, Nicholas Cavenagh\n"
            + "  Concerning eight mutually orthogonal latin squares,\n"
            + "  Vol. 15, n.3, pp. 255-261,\n"
            + "  Journal of Combinatorial Designs, 2007"
...
def construction_q_x(k, q, x, check=True, explain_construction=False):
    ...
    if explain_construction:
        return (
            "(q-x)-construction with q={},x={} from:\n"
            + "   Malcolm Greig,\n"
            + "   Designs from projective planes and PBD bases,\n"
            + "   vol. 7, num. 5, pp. 341--374,\n"
            + "   Journal of Combinatorial Designs, 1999"
    ...
    points_to_delete = set(
        [i * q + j for i in range(x) for j in range(1, q)]
        + [i * q for i in range(x, q)]
    )
...
def thwart_lemma_3_5(
    k, n, m, a, b, c, d=0, complement=False, explain_construction=False
):
    r"""
    ...
    *(When* `d\neq 0` *)*
    ...
    if explain_construction:
        return (
            "Lemma 3.5 with n={},m={},a={},b={},c={},d={} from:\n"
            + "   Charles J.Colbourn, Jeffrey H. Dinitz, Mieczyslaw Wojtas,\n"
            + "   Thwarts in transversal designs,\n"
            + "   Designs, Codes and Cryptography 5, no. 3 (1995): 189-197."
        ).format(n, m, a, b, c, d)
    ...
    assert a <= n and b <= n and c <= n and d <= n, (
        "a,b,c,d (={},{},{},{}) must be <=n(={})".format(a, b, c, d, n)
    )
    assert a + b + c <= n + 1, (
        "{}={}+{}+{}=a+b+c>n+1={}+1 violates the assumptions".format(
            a + b + c, a, b, c, n
        )
    )
    assert k + 3 + bool(d) <= n + 1, "There exists no OA({},{}).".format(
        k + 3 + bool(d), n
    )
    G = GF(n, prefix='x')
    G_set = sorted(G)  # sorted by lexicographic order, G[1] = 1
    assert G_set[0] == G.zero() and G_set[1] == G.one(), (
        "problem with the ordering of {}".format(G)
    )
...
def thwart_lemma_4_1(k, n, m, explain_construction=False):
    ...
    PG = DesarguesianProjectivePlaneDesign(
        q, check=False, point_coordinates=False
    ).blocks()
...
def three_factor_product(k, n1, n2, n3, check=False, explain_construction=False):
    ...
    if explain_construction:
        return (
            "Three-factor product with n={}.{}.{} from:\n"
            + "   Peter J. Dukes, Alan C.H. Ling,\n"
            + "   A three-factor product construction for mutually orthogonal latin squares,\n"
            + "   https://arxiv.org/abs/1401.1466"
        ).format(n1, n2, n3)
    ...
    def assert_c_partition(classs, k, n, c):
        ...
        """
        c = int(c)
        assert all(len(B) == k for B in classs), "A block has length {}!=k(={})".format(
            len(B), k
        )
        assert len(classs) == n * c, "not the right number of blocks"
        for p in zip(*classs):
            assert all(x == i // c for i, x in enumerate(sorted(p))), (
                "A class is not c(={})-parallel".format(c)
            )
    ...
    def product_with_parallel_classes(OA1, k, g1, g2, g1_parall, parall, check=True):
        ...
        for classs2 in g1_parall:
            ...
            for B2 in classs2:
                ...
                for B1 in OA1:
                    copy_of_OA1.append(
                        [x2 * g1 + (x1 + sh) % g1 for sh, x1, x2 in zip(shift, B1, B2)]
                    )
        ...
        for classs2 in parall:
            ...
            for B2 in classs2:
                for B1 in OA1:
                    disjoint_copies_of_OA1.append(
                        [x2 * g1 + x1 for x1, x2 in zip(B1, B2)]
                    )
    ...
    # length n3
    OA3_parall = [
        OA3[i : i + n3] for i in range(len(OA3_n1_parall) * n1 * n3, len(OA3), n3)
    ]

    # First product: OA1 and OA3
    n1_parall, parall = product_with_parallel_classes(
        OA1, k, n1, n3, OA3_n1_parall, OA3_parall, check=check
    )
    ...
    n2_parall, parall = product_with_parallel_classes(
        OA2, k, n2, n1 * n3, n2_parall, parall, check=check
    )
    for n2_classs in n2_parall:
        for i in range(n2):
            partition = [
                ...
                for B in n2_classs[j * n2**2 + i * n2 : j * n2**2 + (i + 1) * n2]
            ]
...
def brouwer_separable_design(
    k, t, q, x, check=False, verbose=False, explain_construction=False
):
    r"""
    ...
    * vi) `t+q<x<q^2+1`
    ...
    if explain_construction:
        return (
            "Brouwer's separable design construction with t={},q={},x={} from:\n"
            + "    Andries E. Brouwer,\n"
            + "    A series of separable designs with application to pairwise orthogonal Latin squares\n"
            + "    Vol. 1, n. 1, pp. 39-41,\n"
            + "    European Journal of Combinatorics, 1980"
    ...
    for i in range(m):
        for B in BIBD:
            assert sum((xx % m) == i for xx in B) in [1, q + 1], sum(
                (xx % m) == i for xx in B
            )
    ...
    for B in BIBD:
        ...
        if plane < t:
            ...
            partition_of_blocks_of_size_t[plane - t].append(
                [relabel[xx] for xx in B if xx % m < t]
            )
    ...
    if x == 0:
        ...
        for PBD_parallel_class in partition_of_blocks_of_size_t:
            for OA_class in OA_t_classes:
                rOA_N_classes.append(
                    [[B[x] for x in BB] for BB in OA_class for B in PBD_parallel_class]
                )
    ...
    # ii)
        ...
        x == t + q
        and orthogonal_array(k + e3, t, existence=True)
        and orthogonal_array(k, t + q, existence=True)
        and orthogonal_array(k + 1, t + q + 1, existence=True)
    ):
        if verbose:
        ...
        if x == q**2:
            ...
            assert len(partition_of_blocks_of_size_t) == 1, (
                "also equivalent to exactly one partition into sets of size t"
            )
            OA = [
                [B[xx] for xx in R]
            ...
            OA = OA_from_PBD(k, N, sum(partition_of_blocks_of_size_t, []), check=False)[
                :-N
            ]
        ...
        OA_tq1 = incomplete_orthogonal_array(
            k, t + q + 1, [1] * (t + q + 1), resolvable=True
        )
        OA_tq1_classes = [
            OA_tq1[i * (t + q + 1) : (i + 1) * (t + q + 1)] for i in range(t + q)
        ]
        ...
        for i, classs in enumerate(OA_tq1_classes):
            OA.extend(
                [R[xx] if xx < t + q else N - i - 1 for xx in B]
    ...
    # iii)
        ...
        x == q**2 - q + 1 - t
        and orthogonal_array(k, x, existence=True)  # d0
        and orthogonal_array(k + e2, t + 1, existence=True)  # d2-e2
        and orthogonal_array(k + 1, t + q, existence=True)
    ):  # d3-e1
        if verbose:
        ...
        if x == 1:
            ...
            OA.extend(
                [B[xx] if xx < t else N - 1 for xx in R]
    ...
    # iv)
        ...
        x == q**2 + 1
        and orthogonal_array(k, x, existence=True)  # d0
        and orthogonal_array(k + e4, t + 1, existence=True)  # d2 - e4
        and orthogonal_array(k + 1, t + q + 1, existence=True)
    ):  # d4 - 1
        if verbose:
        ...
        if e4 == 0:
            ...
            OA = [
                [B[xx] if xx < t else N - x for xx in R]
                for R in incomplete_orthogonal_array(k, t + 1, [1])
                for B in partition_of_blocks_of_size_t[0]
            ]
        else:
    ...
    # v)
        ...
        0 < x
        and x < q**2 - q + 1 - t
        and (e1 or e2)  # The result is wrong when e1=e2=0
        and orthogonal_array(k, x, existence=True)  # d0
        and orthogonal_array(k + e1, t, existence=True)  # d1-e1
        and orthogonal_array(k + e2, t + 1, existence=True)  # d2-e2
        and orthogonal_array(k + 1, t + q, existence=True)
    ):  # d3-1
        if verbose:
            print(
                "Case v) with k={},q={},t={},x={},e1={},e2={}".format(
                    k, q, t, x, e1, e2
                )
            )
        ...
        if e2:
            ...
            OA.extend(
                OA_from_PBD(
                    k, N, sum(partition_of_blocks_of_size_t[:x], []), check=False
                )[:-N]
            )
        ...
        if e1 == 0 and e2 == 0:
            raise RuntimeError(
                "Brouwer's construction does not work for case v) with e2=e1=0"
            )
    ...
    # vi)
        ...
        t + q < x
        and x < q**2 + 1
        and (e3 or e4)  # The result is wrong when e3=e4=0
        and orthogonal_array(k, x, existence=True)  # d0
        and orthogonal_array(k + e3, t, existence=True)  # d1-e3
        and orthogonal_array(k + e4, t + 1, existence=True)  # d2-e4
        and orthogonal_array(k + 1, t + q + 1, existence=True)
    ):  # d4-1
        if verbose:
            print(
                "Case vi) with k={},q={},t={},x={},e3={},e4={}".format(
                    k, q, t, x, e3, e4
                )
            )
        ...
        if e4:
            ...
            OA.extend(
                OA_from_PBD(
                    k,
                    N,
                    sum(partition_of_blocks_of_size_t[: x - (q + t)], []),
                    check=False,
                )[:-N]
            )
        else:
        ...
        if e3 and e4:
            ...
            raise RuntimeError(
                "Brouwer's construction does not work for case v) with e3=e4=0"
            )
