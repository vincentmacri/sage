...
def hochschild_lattice(n) -> FiniteLatticePoset:
    ...
    dg = DiGraph({a: list(cover_relations(a)) for a in verts}, format="dict_of_lists")
    return LatticePoset(
        dg.reverse(),
...
def hochschild_fan(n):
    ...
    rays = [vector([1] * n)]
    rays.extend(vector([0 if j != i else -1 for j in range(n)]) for i in range(n))

    cones = [Cone([rays[j] for j in range(n + 1) if j != i]) for i in range(n + 1)]
