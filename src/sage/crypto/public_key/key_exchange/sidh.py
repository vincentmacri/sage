...
class SIDH(KeyExchangeBase):
    ...
    def secret_isogeny_path(self,
        ...
        with kernel generated by ``P + secret_key * Q`` and ``path`` is is
        a tuple of integers corresponding to the j-invariant of each isomorphism
        class of the curves on the isogeny path.
        """
