    ...
    def _element_constructor_(self, x, check=True):
        ...
        if not isinstance(x, GapElement):
            try:
                ...
                pass
        f = AbelianGroupAutomorphismGroup_subgroup._element_constructor_(
            self, x, check=True
        )
        if check:
    ...
    def _get_action_(self, S, op, self_on_left):
        ...
        if op == operator.mul and not self_on_left:
            ...
            try:
                ...
                elif S.V().is_submodule(T.V()) and T.W().is_submodule(
                    S.W()
                ):  # is a subquotient
                    Q1 = S.V() / T.W()
                    Q2 = S.W() / T.W()
                    if all(
                        T(q) * g in Q1 for q in Q1.gens() for g in self.gens()
                    ) and all(T(q) * g in Q2 for q in Q2.gens() for g in self.gens()):
                        return ActionOnFqf(self, S, on_subquotient=True)
            except AttributeError:
                pass
            try:
                return AbelianGroupAutomorphismGroup_subgroup._get_action_(
                    self, S, op, self_on_left
                )
            except AttributeError:
                pass
...
def _isom_fqf(A, B=None):
    ...
    def orbits(G, L):
        ...
        orb = [
            [D.linear_combination_of_smith_form_gens(A(g).exponents()) for g in f]
            for f in orb
        ]
        return orb
    ...
    b_cand = [
        [b for b in B if b.q() == a.q() and b.order() == a.order()]
        for a in A.smith_form_gens()
    ]
    ...
    while len(waiting) > 0:
        ...
        if i == n:
            ...
            if g not in G:
                G = B.orthogonal_group(
                    tuple(ambient(s.matrix()) for s in G.gens()) + (g,)
                )
