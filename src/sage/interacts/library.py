...
def taylor_polynomial(title, f, order: int):
    ...
    html(r'$\hat{f}(x;%s)\;=\;%s+\mathcal{O}(x^{%s})$' % (x0, latex(ft), order + 1))
...
def definite_integral(title, f, g, interval, x_range, selection):
    ...
    if selection == "f" or selection == "f and g":
        f_plot += plot(
            f(x), x, interval, color='blue', fill=True, fillcolor='blue', fillalpha=0.15
        )
        text += (
            r"$\int_{%.2f}^{%.2f}(\color{Blue}{f(x)})\,\mathrm{d}x=\int_{%.2f}^{%.2f}(%s)\,\mathrm{d}x=%.2f$"
            % (
                interval[0],
                interval[1],
                interval[0],
    ...
    if selection == "g" or selection == "f and g":
        ...
        text += (
            r"$\int_{%.2f}^{%.2f}(\color{Green}{g(x)})\,\mathrm{d}x=\int_{%.2f}^{%.2f}(%s)\,\mathrm{d}x=%.2f$"
            % (
                interval[0],
                interval[1],
                interval[0],
    ...
    if selection == "f - g":
        g_plot = plot(g(x), x, x_range, color='green', thickness=1.5)
        g_plot += plot(
            g(x), x, interval, color='green', fill=f(x), fillcolor='red', fillalpha=0.15
        )
        h_plot = plot(
            f(x) - g(x),
            x,
        ...
        text = (
            r"$\int_{%.2f}^{%.2f}(\color{Red}{f(x)-g(x)})\,\mathrm{d}x=\int_{%.2f}^{%.2f}(%s)\,\mathrm{d}x=%.2f$"
            % (
                interval[0],
                interval[1],
                interval[0],
...
def function_derivative(title, function, x_range, y_range):
    ...
    ddf = derivative(df, x)
    plots = (
        plot(f(x), x_range, thickness=1.5)
        + plot(df(x), x_range, color='green')
        + plot(ddf(x), x_range, color='red')
    )
    if y_range == (0, 0):
...
def difference_quotient(title, f, interval, a, x0):
    ...
    measure_1 = line2d(
        [(x0, measure_y + 0.02 * f_height), (x0, measure_y - 0.02 * f_height)],
        rgbcolor='black',
    )
    measure_2 = line2d(
        [(a, measure_y + 0.02 * f_height), (a, measure_y - 0.02 * f_height)],
        rgbcolor='black',
    )
    ...
    dashline = line2d(
        [(x0, f(x0)), (x0, f(a)), (a, f(a))], rgbcolor='#005500', linestyle='--'
    )
    html('<h2>Difference Quotient</h2>')
    show(
        fplot + tanplot + points + dashline + measure,
        xmin=interval[0],
        xmax=interval[1],
        ymin=fmin - 0.2 * f_height,
        ymax=fmax,
    )
    ...
    html(r"$\text{Slope:}$")
    html(
        r"$k = \frac{f(x_0)-f(a)}{x_0-a} = %s$<br>"
        % (N(derivative(tanf(x), x), digits=5))
    )
...
def trigonometric_properties_triangle(a0, a1, a2):
    ...
    ak = [
        angle(al[0], al[1], al[2]),
        angle(al[1], al[2], al[0]),
        angle(al[2], al[0], al[1]),
    ]
    ...
    show(
        unit_circle + triangle + triangle_points + labels,
        figsize=[5, 5],
        xmin=-1,
        xmax=1,
        ymin=-1,
    ...
    html(
        r"$\angle A = {%.3f}^{\circ},$ $\angle B = {%.3f}^{\circ},$ $\angle C = {%.3f}^{\circ}$"
...
def unit_circle(function, x):
    ...
    t = SR.var('t')
    html(
        '<div style="white-space: normal;">Lines of the same color have\
         the same length</div>'
    )
    ...
    C_inner = parametric_plot(
        (cos(t), sin(t)), (t, 0, x + 0.001), color='green', thickness=3
    )
    C_outer = parametric_plot(
        (0.1 * cos(t), 0.1 * sin(t)), (t, 0, x + 0.001), color='black'
    )
    ...
    # Tangent
        ...
        Cf_line2 = line(
            [(xy[0], xy[1]), (1, tan(x))], rgbcolor='purple', linestyle='--'
        )
...
def special_points(
    ...
    def half(A, a, b, c):
        if (A[a] < A[b] and (A[c] < A[a] or A[c] > A[b])) or (
            A[a] > A[b] and (A[c] > A[a] or A[c] < A[b])
        ):
            p = A[a] + 0.5 * (A[b] - A[a])
        else:
    ...
    if show_alt:
        ...
        a_alt = plot(
            ((xC - xB) * x + (xB - xC) * xA) / (yB - yC) + yA,
            (x, -3, 3),
            rgbcolor='brown',
            alpha=0.6,
        )
        b_alt = plot(
            ((xA - xC) * x + (xC - xA) * xB) / (yC - yA) + yB,
            (x, -3, 3),
            rgbcolor='brown',
            alpha=0.6,
        )
        c_alt = plot(
            ((xB - xA) * x + (xA - xB) * xC) / (yA - yB) + yC,
            (x, -3, 3),
            rgbcolor='brown',
            alpha=0.6,
        )
...
def coin(n, interval):
    ...
    for i in range(1, n + 1):
        ...
        c.append((i, k / i))
    show(
        point(c[1:], gridlines=[None, [0.5]], pointsize=1),
        ymin=interval[0],
        ymax=interval[1],
    )
...
def bisection_method(title, f, interval, d, maxn):
    ...
    def _bisection_method(f, a, b, maxn, eps):
        ...
        while True:
            ...
            if fa * fc < 0:
                ...
                raise ValueError(
                    "f must have a sign change in the interval (%s,%s)" % (a, b)
                )
    ...
    try:
        ...
        L += sum(
            line([(c, k * i - k / 4), (c, k * i + k / 4)])
            for i, (c, d) in enumerate(intervals)
        )
        L += sum(
            line([(d, k * i - k / 4), (d, k * i + k / 4)])
            for i, (c, d) in enumerate(intervals)
        )
...
def secant_method(title, f, interval, d, maxn):
    ...
    if float(f(a) * f(b)) > 0:
        ...
        S = sum(
            line(
                [(c, f(c)), (d, f(d)), (d - (d - c) * f(d) / (f(d) - f(c)), 0)],
                color='green',
            )
...
def newton_method(title, f, c, d, maxn, interval, list_steps):
    ...
    if list_steps:
        ...
        for i in range(len(midpoints) - 1):
            L += line(
                [(midpoints[i], f(midpoints[i])), (midpoints[i + 1], 0)], color='red'
            )
...
def trapezoid_integration(
    ...
    for i in range(n):
        ...
        yi = f(xi)
        trapezoids += line(
            [[xi, 0], [xi, yi], [xi + h, f(xi + h)], [xi + h, 0], [xi, 0]],
            rgbcolor=(1, 0, 0),
        )
        xs.append(xi)
        ys.append(yi)
    ...
    show(
        plot(f, interval[0], interval[1]) + trapezoids,
        xmin=interval[0],
        xmax=interval[1],
    )
    ...
    html(
        r'Integral value to seven decimal places is: $\displaystyle\int_{%.2f}^{%.2f} {f(x) \, \mathrm{d}x} = %.6f$'
        % (interval[0], interval[1], N(numeric_value, digits=7))
    )

    if output_form == 'traditional':
        sum_formula_html = (
            r"\frac {d}{2} \cdot \left[f(x_0) + %s + f(x_{%s})\right]"
            % (' + '.join(["2 f(x_{%s})" % i for i in range(1, n)]), n)
        )
        sum_placement_html = (
            r"\frac{%.2f}{2} \cdot \left[f(%.2f) + %s + f(%.2f)\right]"
            % (
                h,
                N(xs[0], digits=5),
...
def simpson_integration(
    ...
    for i in range(0, n - 1, 2):
        ...
        lines += line(
            [(xs[i + 1], ys[i + 1]), (xs[i + 1], 0)], linestyle='-.', color='red'
        )
    ...
    show(
        plot(f(x), x, interval[0], interval[1]) + parabolas + lines,
        xmin=interval[0],
        xmax=interval[1],
    )
    ...
    approx = (
        ...
        * (
            ys[0]
            + sum([4 * ys[i] for i in range(1, n, 2)])
            + sum([2 * ys[i] for i in range(2, n, 2)])
            + ys[n]
        )
    )
    ...
    if output_form == 'traditional':
        sum_formula_html = (
            r"\frac{d}{3} \cdot \left[ f(x_0) + %s + f(x_{%s})\right]"
            % (
                ' + '.join(
                    r"%s \cdot f(x_{%s})" % (i % 2 * (-2) + 4, i + 1)
                    for i in range(n - 1)
                ),
                n,
            )
        ...
        sum_placement_html = (
            r"\frac{%.2f}{3} \cdot \left[ f(%.2f) +  %s + f(%.2f)\right]"
            % (
                dx,
                N(xs[0], digits=5),
                ' + '.join(
                    r"%s \cdot f(%.2f)" % (i % 2 * (-2) + 4, N(xk, digits=5))
        ...
        sum_values_html = r"\frac{%.2f}{3} \cdot \left[ %s %s %s\right]" % (
            ...
            "%.2f + " % N(ys[0], digits=5),
            ' + '.join(
                r"%s \cdot %.2f" % (i % 2 * (-2) + 4, N(yk, digits=5))
        ...
        pretty_print(table(s, header_row=True))
        html(
            r'$\int_{%.2f}^{%.2f} {f(x) \, \mathrm{d}x}\approx\frac {%.2f}{3}\cdot %s=%s$'
...
def riemann_sum(
    ...
    for i in range(n):
        ...
        rects = (
            rects
            + polygon2d(body, rgbcolor=color_rect, alpha=0.1)
    ...
    html(
        r'Riemann sum: $\displaystyle\sum_{i=1}^{%s} f(\eta_i)(x_i-x_{i-1})=%s$ '
        % (latex(n), latex(sum([ys[i] * delka_intervalu[i] for i in range(n)])))
    )
    html(
        r'Exact value of the integral $\displaystyle\int_{%s}^{%s}%s\,\mathrm{d}x=%s$'
        % (
            latex(a),
            latex(b),
            latex(func(x)),
            latex(integral_numerical(func(x), a, b)[0]),
        )
    )
...
def function_tool(f, g, xrange, yrange, a, action, do_plot):
    ...
    try:
        ...
        print(msg[-200:])
        print(
            "Unable to make sense of f,g, or a as symbolic expressions in single variable x."
        )
        return
    ...
    html(
        '<center><font color="blue"><b>$h = %s = %s$</b></font></center>'
        % (lbl, latex(h))
    )
    if do_plot:
        P = (
            plot(f, xrange, color='red', thickness=2)
...
def julia(expo, c_real, c_imag, iterations, zoom_x, zoom_y, plot_points, dpi):
    ...
    html(
        r'Recursive Formula: $z \leftarrow z^{%.2f} + (%.2f+%.2f*\mathbb{I})$'
        % (expo, c_real, c_imag)
    )
    complex_plot(
        lambda z: julia(ff_j, z, iterations),
        zoom_x,
        zoom_y,
        plot_points=plot_points,
        dpi=dpi,
    ).show(frame=True, aspect_ratio=1)
...
def mandelbrot(expo, iterations, zoom_x, zoom_y, plot_points, dpi):
    ...
    html(
        r'Recursive Formula: $z \leftarrow z^{%.2f} + c$ for $c \in \mathbb{C}$' % expo
    )
    complex_plot(
        lambda z: mandel(ff_m, z, iterations),
        zoom_x,
        zoom_y,
        plot_points=plot_points,
        dpi=dpi,
...
def polar_prime_spiral(interval, show_factors, highlight_primes, show_curves, n, dpi):
    ...
    if not show_factors:
        ...
        for i in srange(start, end, include_endpoint=True):
            ...
            list.append(
                disk((f(i - start + 1)), 0.05 * pow(2, len(factor(i)) - 1), (0, 2 * pi))
            )
        ...
        if not highlight_primes:
            list2 = [(f(n - start + 1))]
        R = points(list2, hue=0.1, pointsize=p_size)
    ...
    if n > 0:
        ...
        W1 = disk((f(n - start + 1)), p, (pi / 6, 2 * pi / 6), alpha=0.1)
        W2 = disk((f(n - start + 1)), p, (4 * pi / 6, 5 * pi / 6), alpha=0.1)
        W3 = disk((f(n - start + 1)), p, (7 * pi / 6, 8 * pi / 6), alpha=0.1)
        W4 = disk((f(n - start + 1)), p, (10 * pi / 6, 11 * pi / 6), alpha=0.1)
        Q = W1 + W2 + W3 + W4

        n -= (
            start - 1
        )  # offset n for different start values to ensure accurate plotting
        if show_curves:
            begin_curve = 0
            t = SR.var('t')
            a = 1.0
            ...
            S1 = parametric_plot(
                ((r(t)) * cos(2 * pi * (theta(t))), (r(t)) * sin(2 * pi * (theta(t)))),
