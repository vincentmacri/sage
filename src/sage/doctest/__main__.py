...
def _make_parser():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="timeout (in seconds) for doctesting one file, 0 for no timeout",
    )
    ...
    what.add_argument(
        ...
        action="store_true",
        default=False,
        help="test all installed modules of the Sage library",
    )
    parser.add_argument(
        "--logfile",
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="include lines with the phrase 'long time'",
    )
    parser.add_argument(
        "-s",
        "--short",
        dest="target_walltime",
        nargs='?',
        type=int,
        default=-1,
        const=300,
    ...
    parser.add_argument(
        "--optional",
        metavar="FEATURES",
        default=_get_optional_defaults(),
    ...
    parser.add_argument(
        "--randorder", type=int, metavar="SEED", help="randomize order of tests"
    )
    parser.add_argument(
        "--random-seed",
        dest="random_seed",
        type=int,
        metavar="SEED",
        help="random seed (integer) for fuzzing doctests",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="repeat the whole testing process this many times",
    )
    parser.add_argument(
        "--file-iterations",
        "--file_iterations",
        type=int,
        default=0,
        help="repeat each file this many times, stopping on the first failure",
    )
    parser.add_argument(
        "--environment",
        type=str,
        default="sage.repl.ipython_kernel.all_jupyter",
        help="name of a module that provides the global environment for tests",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="only show the first failure in each file",
    )
    parser.add_argument(
        "--exitfirst",
        action="store_true",
        default=False,
        help="end the test run immediately after the first failure or unexpected exception",
    )
    parser.add_argument(
        "--force_lib",
        "--force-lib",
        action="store_true",
        default=False,
        help="do not import anything from the tested file(s)",
    )
    parser.add_argument(
        "--if-installed",
        action="store_true",
        default=False,
        help="skip Python/Cython files that are not installed as modules",
    )
    parser.add_argument(
        "--abspath",
        action="store_true",
        default=False,
        help="print absolute paths rather than relative paths",
    )
    parser.add_argument(
        "--verbose",
        action="store_true",
        default=False,
        help="print debugging output during the test",
    )
    parser.add_argument(
        "-d",
        "--debug",
        action="store_true",
        default=False,
        help="drop into a python debugger when an unexpected error is raised",
    )
    parser.add_argument(
        "--only-errors",
        action="store_true",
        default=False,
        help="only output failures, not test successes",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="run doctests under the control of gdb",
    )
    parser.add_argument(
        "--lldb",
        action="store_true",
        default=False,
        help="run doctests under the control of lldb",
    )
    parser.add_argument(
        "--valgrind",
        "--memcheck",
    ...
    parser.add_argument(
        "--stats_path",
        "--stats-path",
        default=os.path.join(DOT_SAGE, "timings2.json"),
    ...
    # document it.
    parser.add_argument(
        "--serial", action="store_true", default=False, help=argparse.SUPPRESS
    )
...
def main():
    ...
    for arg in sys.argv[1:]:
        if arg in ('-n', '--new', '-a', '--all', '--installed') or arg.startswith(
            "--all-except"
        ):
    ...
    if (
        not args.filenames
        and not (args.all or args.new or args.installed)
        and args.all_except is None
    ):
        print(
            'either use --new, --all, --all-except=..., --installed, or some filenames'
        )
        return 2
    ...
    if pytest_nprocs != 1 and not find_spec("xdist"):
        ...
        warn(
            "pytest-xdist not found, pytest will run sequentially", pytest.PytestWarning
        )
        pytest_nprocs = 1
