...
class UniqueSimplicialComplex(SimplicialComplex, UniqueRepresentation):
    ...
    def __init__(self, maximal_faces=None, name=None, **kwds):
        ...
        SimplicialComplex.__init__(
            self, maximal_faces=maximal_faces, immutable=True, **kwds
        )
...
def Sphere(n):
    ...
    return UniqueSimplicialComplex(
        facets, name='Minimal triangulation of the {}-sphere'.format(n)
    )
...
def Simplex(n):
    ...
    EXAMPLES::
        ...
        1
    """
    return UniqueSimplicialComplex([TrueSimplex(n)], name='The {}-simplex'.format(n))
...
def MooreSpace(q):
    ...
    return UniqueSimplicialComplex(
        facets, name='Triangulation of the mod {} Moore space'.format(q)
    )
...
def RealProjectiveSpace(n):
    ...
    if n >= 5:
        ...
        return UniqueSimplicialComplex(
            list(facets), name='Triangulation of RP^{}'.format(n)
        )
...
def NotIConnectedGraphs(n, i):
    ...
    return UniqueSimplicialComplex(
        facets,
        name='Simplicial complex of not {}-connected graphs on {} vertices'.format(
            i, n
        ),
    )
        ...
        if is_even(n):
            ...
            for w in Xcomp:
                ...
                for M in matching(A, B):
                    ...
                    facets.append(facet)
    return UniqueSimplicialComplex(
        facets, name='Matching complex on {} vertices'.format(n)
    )
...
def ChessboardComplex(n, i):
    ...
    for M in matching(A, B):
        ...
        facets.append(facet)
    return UniqueSimplicialComplex(
        facets, name='Chessboard complex for an {}x{} chessboard'.format(n, i)
    )
...
def SumComplex(n, A):
    ...
    return UniqueSimplicialComplex(
        facets, name='Sum complex on vertices Z/{}Z associated to {}'.format(n, Set(A))
    )
...
def RandomTwoSphere(n):
    ...
    triangles = [
        (u, v, w)
        for u, L in graph.get_embedding().items()
        for v, w in zip(L, L[1:] + [L[0]])
        if u < v and u < w
    ]
...
def FareyMap(p):
    ...
    points = [
        ...
        for x in range(p)
        for y in range(p)
        if (x, y) != (0, 0) and (x != 0 and p - x >= x or (x == 0 and p - y >= y))
    ]
