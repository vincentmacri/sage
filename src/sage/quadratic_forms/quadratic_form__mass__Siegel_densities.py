...
def Watson_mass_at_2(self):
    ...
    diag_dict = dict.fromkeys(range(s_min - 2, s_max + 4), Null_Form)     # Initialize with the zero form
    dim2_dict = dict.fromkeys(range(s_min, s_max + 4), Null_Form)       # Initialize with the zero form
    ...
    n_dict = dict.fromkeys(range(s_min + 1, s_max + 2), 0)
    m_dict = dict.fromkeys(range(s_min, s_max + 4), 0)
    for s, L in Jordan_Blocks:
        n_dict[s + 1] = L.dim()
