...
def K2_cp_cone(K2, K2_cp_gens):
    ...
    return Cone((g.list() for g in K2_cp_gens), lattice=L, check=False)
...
def K2_posops_cone(K2, K2_posops_gens):
    L = ToricLattice(K2.lattice_dim() ** 2)
    return Cone((g.list() for g in K2_posops_gens), lattice=L, check=False)


@pytest.fixture
def K2_K3_posops_cone(K2, K3, K2_K3_posops_gens):
    L = ToricLattice(K2.lattice_dim() * K3.lattice_dim())
    return Cone((g.list() for g in K2_K3_posops_gens), lattice=L, check=False)
...
def test_random_element_membership(K):
    ...
    e1 = sum((K.random_element() for _ in range(randint(0, 10))), start=L.zero())
    e2 = sum((K.random_element(ring=F) for _ in range(randint(0, 10))), start=L.zero())
...
def test_reducibility_criteria(K, J, P, Q, K2, K3):
    ...
    """
    winners = (C for C in (P, Q, K, J, K2, K3) if not C.is_trivial() and C.is_proper())
    try:
        C = next(winners)
    except StopIteration:
        C = random_cone(
            strictly_convex=True, solid=True, max_ambient_dim=5, min_rays=1, max_rays=7
        )
...
def test_is_positive(K2, K3, K2_K3_posops_gens):
    ...
    assert all(
        A.change_ring(SR).is_positive_operator_on(K2, K3) for A in K2_K3_posops_gens
    )
...
def test_is_lyapunov_like(K2, K2_ll_basis):
    ...
    assert all(A.change_ring(SR).is_lyapunov_like_on(K2) for A in K2_ll_basis)
...
def test_cross_positive_permutation(K2, K2_cp_gens):
    ...
    actual = Cone(
        (g.list() for g in pK2.cross_positive_operators_gens()), lattice=L, check=False
    )
    expected = Cone(
        ((p * g * p.inverse()).list() for g in K2_cp_gens), lattice=L, check=False
    )
...
def test_cross_positive_adjoint(K2, K2_cp_cone):
    ...
    cp_star = Cone((g.list() for g in dual_gens), lattice=L, check=False)
...
def test_posops_dual_linspace(K2):
    ...
    U1 = (V((s.tensor_product(x)).list()) for x in K2.lines() for s in K2.dual())
    U2 = (V((s.tensor_product(x)).list()) for x in K2 for s in K2.dual().lines())
...
def test_posops_on_element(K2, K3, K2_K3_posops_cone):
    ...
    F = K2.lattice().base_field()
    P = matrix(
        K3.lattice_dim(), K2.lattice_dim(), K2_K3_posops_cone.random_element(F).list()
    )
...
def test_posops_permutation(K2, K2_posops_gens):
    ...
    actual = Cone(
        (g.list() for g in pK.positive_operators_gens()), lattice=L, check=False
    )
    expected = Cone(
        ((p * g * p.inverse()).list() for g in K2_posops_gens), lattice=L, check=False
    )
    assert actual.is_equivalent(expected)
...
def test_posops_adjoint(K2, K3, K2_K3_posops_cone):
    ...
    pi_back = Cone((g.list() for g in pi_back_gens), lattice=L, check=False)
...
def test_posops_LL(K2, K3, K2_K3_posops_cone):
    ...
    tps = (
        M2(s.list()).tensor_product(M1(x.list()))
...
def test_LL_closed_under_lie_bracket(K2, K2_ll_basis):
    ...
    brackets = (
        W((A1 * A2 - A2 * A1).list()) for A1 in K2_ll_basis for A2 in K2_ll_basis
    )
