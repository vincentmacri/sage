    ...
    if hyperplane.dimension() == 0:  # a point on a line
        ...
        if has_hyp_label:
            if not has_offset:
                label_offset = 0.1
            p += text(
                label, (d / x, label_offset), color=label_color, fontsize=label_fontsize
            )
        ...
        if ranges_set:
            ...
            t0, t1 = -3, 3
        p = parametric_plot3d(
            pnt + s * w[0] + t * w[1], (s, s0, s1), (t, t0, t1), **kwds
        )
        if has_hyp_label:
            if has_offset:
...
def legend_3d(hyperplane_arrangement, hyperplane_colors, length):
    ...
    if length == 'short':
        ...
        labels = ['  ' + hyps[i]._repr_linear(include_zero=False) for i in range(N)]
    p = Graphics()
    for i in range(N):
        p += line(
            [(0, 0), (0, 0)],
